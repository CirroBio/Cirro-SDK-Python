        ...
        *meta* is a `dict` of extracted values:
        ...
        Args:
            ...
            filetype (str): File format used to parse each file. Supported values:
                ...
                - `'parquet'`: parse with `pandas.read_parquet()`, returns a `DataFrame`
                  (requires `pyarrow` or `fastparquet`)
                - `'feather'`: parse with `pandas.read_feather()`, returns a `DataFrame`
                  (requires `pyarrow`)
                - `'pickle'`: deserialize with `pickle`, returns a Python object
                - `'excel'`: parse with `pandas.read_excel()`, returns a `DataFrame`
